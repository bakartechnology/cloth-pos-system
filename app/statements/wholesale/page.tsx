...
export default function WholesaleStatementPage() {
  ...
  // Two clearly separated categories/tabs: Customer vs Client
  const [activeTab, setActiveTab] = useState<'Customer' | 'Client'>('Client');
  ...
  useEffect(() => {
    ...
    const wholesaleBills = salesService.getAllBills().filter(b => b.saleType === 'Wholesale');
    // 2. Apply 5-year rolling retention policy safely
    const retained = retentionService.filterActiveStatements(wholesaleBills);
    setBills(retained);
  }, []);
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200/80 shadow-2xs">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="p-1 rounded bg-cyan-50 text-cyan-700 font-mono text-[10px] font-bold uppercase">
                  Wholesale Ledger & Audit
                </span>
                <span className="text-xs text-slate-500 font-medium">5-Year Rolling Retention</span>
              </div>
              ...
              <p className="text-xs text-slate-500">
                Independent statement registries for registered Wholesale Clients and ad-hoc Wholesale Customers.
              </p>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <Printer className="w-4 h-4 text-cyan-600" /> Print Statement
              </Button>
            </div>
          </div>

          {/* TWO CLEARLY SEPARATED TABS: Customer vs Client */}
          <div className="flex p-1.5 bg-slate-100/80 rounded-2xl border border-slate-200 w-full sm:w-fit">
            <button
              type="button"
            ...
            >
              ...
              <span>Wholesale Client Statement</span>
              <Badge size="sm" variant="default" className="text-[10px] bg-cyan-100 text-cyan-800 ml-1">
                {bills.filter(b => Boolean(b.clientId || b.clientName)).length}
              </Badge>
            </button>
            ...
            >
              ...
              <span>Wholesale Customer Statement</span>
              <Badge size="sm" variant="secondary" className="text-[10px] ml-1">
                {bills.filter(b => !b.clientId).length}
              </Badge>
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
            <Card className="p-4">
              <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                {activeTab === 'Client' ? 'Client Invoices' : 'Customer Invoices'}
              </span>
            ...
            <Card className="p-4">
              ...
              <div className="text-xs text-slate-400 mt-0.5">Wholesale net amount</div>
            </Card>
            ...
            <Card className="p-4">
              ...
              <div className="text-xs text-slate-400 mt-0.5">Wholesale tier discounts</div>
            </Card>
            ...
            <Card className="p-4">
              ...
              <div className="text-xs text-slate-400 mt-0.5">Received payments</div>
            </Card>
          </div>
          ...
          <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-2xs flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="relative w-full md:w-96">
              ...
              <Input
                placeholder={`Search ${activeTab === 'Client' ? 'client business, contact' : 'customer name'}, phone, invoice #, fabric...`}
                value={searchQuery}
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="text-xs font-bold text-slate-500 mr-1 flex items-center gap-1">
                <Calendar className="w-3.5 h-3.5" /> 5-Yr Retention:
              </span>
              <button
                type="button"
                ...
                <thead className="bg-slate-50 border-b border-slate-200 text-slate-600 font-bold uppercase tracking-wider text-[11px]">
                  <tr>
                    ...
                    <th className="px-4 py-3.5">
                      {activeTab === 'Client' ? 'Client Business Name' : 'Customer Name'}
                    </th>
                    ...
                    <th className="px-4 py-3.5 font-black text-slate-900">Total Amount</th>
                    ...
                    <tr>
                      <td colSpan={9} className="text-center py-12 text-slate-400">
                        ...
                        <p className="font-semibold text-sm text-slate-600">
                          No {activeTab.toLowerCase()} wholesale statements found
                        </p>
                        <p className="text-xs mt-1">Try selecting the other tab or changing the retention period.</p>
                      </td>
                    </tr>
                  ) : (
                        ...
                        <tr key={bill.id} className="hover:bg-slate-50/70 transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <div className="font-bold text-slate-900 flex items-center gap-1.5">
                              ...
                              {activeTab === 'Client'
                                ? (bill.clientName || bill.customerName)
                                : (bill.customerName || 'Wholesale Buyer')}
                            </div>
                            {bill.customerPhone && (
                              <div className="text-[10px] text-slate-400 font-mono">
                                {bill.customerPhone}
          ...
          {/* MODAL: View Wholesale Invoice Details */}
          <Modal
            isOpen={!!selectedBill}
            onClose={() => setSelectedBill(null)}
            title="Wholesale Transaction Statement Details"
            maxWidth="2xl"
          >
            {selectedBill && (
              <div className="space-y-4 text-xs">
                ...
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 grid grid-cols-2 sm:grid-cols-4 gap-3 text-[11px]">
                  <div>
                    ...
                    <span className="font-mono font-black text-cyan-800">{selectedBill.invoiceNumber}</span>
                  </div>
                  <div>
                  ...
                  <div>
                    <span className="text-slate-400 block font-bold uppercase text-[10px]">
                      {selectedBill.clientName ? 'Client Business' : 'Customer'}
                    </span>
                    <span className="font-bold text-slate-900">
                      {selectedBill.clientName || selectedBill.customerName}
                    </span>
                  </div>
                  <div>
                    <span className="text-slate-400 block font-bold uppercase text-[10px]">Sales Officer</span>
                  ...
                  <table className="w-full text-xs text-left">
                    <thead className="bg-slate-100/70 border-b border-slate-200 text-slate-600 font-bold uppercase text-[10px]">
                      <tr>
                        ...
                        <th className="px-3 py-2 text-right">Extra Discount</th>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {selectedBill.items.map((item, idx) => (
                        <tr key={idx} className="hover:bg-slate-50/50">
                          ...
                          <td className="px-3 py-2 text-right font-mono text-cyan-700 font-semibold">
                            {item.extraDiscountRupees ? `Rs. ${item.extraDiscountRupees.toLocaleString()}` : item.discountPercent ? `${item.discountPercent}%` : '—'}
                          </td>
                ...
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-1.5">
                  ...
                  <div className="flex justify-between text-sm font-black text-slate-900 pt-1 border-t border-slate-200">
                    ...
                    <span className="text-cyan-800 font-mono">Rs. {selectedBill.grandTotal.toLocaleString()}</span>
                  </div>
                ...
                <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
                  ...
                  >
                    <Printer className="w-3.5 h-3.5" /> Print Statement Invoice
                  </Button>
                </div>
              </div>
            )}
